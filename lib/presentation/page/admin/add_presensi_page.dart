...
class AddPresensiPasienPage extends StatefulWidget {
  const AddPresensiPasienPage(
      {super.key, required this.kegiatan, required this.role});
  final Map kegiatan;
  final String role;

  @override
  State<AddPresensiPasienPage> createState() => _AddPresensiPasienPageState();
}

class _AddPresensiPasienPageState extends State<AddPresensiPasienPage> {
  ...
  menuOption(String value, User user) async {
    if (value == 'hadir') {
      bool success = await SourceUser.addPresensi(
          user.idUser!, widget.kegiatan['id_kegiatan']);
      if (success) {
        cPresensi.getList(widget.kegiatan['id_kegiatan']);
      }
    }
